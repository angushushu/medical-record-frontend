    ...
    <div style="max-width:59rem !important; margin-left:auto !important; margin-right:auto !important;">
        ...
        <el-row>
            <el-table :data="filterTableData" style="width: 100%">
                ...
                <el-table-column label="homepage_id" prop="homepage_id" />
...
const tableData: Settlement[] = reactive([])
const query = ref('')
...
const updateData = onMounted(async ()=>{
    ...
    await axios
        .get('/api/v1/get-settlements/',{params:{page:current_page.value, query:query.value}})
        .then(response=>{
            tableData.length = 0 // 清空
            response.data.settlements.forEach(e=>{
                // 针对空变量写空 （仅测试）
                if(e.age === "请先输入其他信息")
                    e.age = ''
                tableData.push(e)
...
const handleEdit = (index: number, row: Settlement) => {
    console.log(index, row)
    router.push('/settlement/'+row.homepage_id)
}
const handleDelete = async (index: number, row: Settlement) => {
    console.log(row.homepage_id)
    await axios
        .get('/api/v1/del-settlement/',{params:{homepage_id:row.homepage_id}})
        .then(response=>{ // 可以改一下
